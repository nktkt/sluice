...
**VERIFY レベル**: (1) 構造（全参照 id が index に解決、DAG 完全到達、dangling なし、roaring bitmap で有界）→ (2) presence（全参照 pack を stat、期待サイズ）→ (3) `--read-data`（全 pack DL、pack-id 再計算、各 blob 復号→解凍→keyed-BLAKE3 再計算==id）→ (4) `--read-data-subset N/M`（`rand_chacha` シードで回転サンプリング、低 egress 継続スクラブ、Cairn graft）。
