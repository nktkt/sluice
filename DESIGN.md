...
> **注**: 以下の表は**当初の設計サーフェス**（理想形）であり、出荷済みの実 CLI とは一部異なる。
> 実装では repo は**位置引数**（グローバル `--repo` ではない）、sparse リストアは（フラグではなく）
> バックアップ時に記録した sparseness から**自動**、`verify` は既定で read-data・構造専用チェックは
> 別コマンド `check`、破壊的操作の確認は対話 `--yes` ではなく `--dry-run` プレビュー、という具合。
> 出荷済みコマンド/フラグの一次情報は **README** と §11 のステータス注記を参照。
...
> M0–M9 は実質的に出荷済み。**コア**: FastCDC dedup、zstd（skip-if-incompressible、`init --compression`）、
> Argon2id/XChaCha20-Poly1305、keyed-BLAKE3 id、S3 系オブジェクトストア、複数パスフレーズ
> （`key add|list|passwd|remove`、アクティブ鍵追跡）、永続インデックス（高速 open、`rebuild-index`）、
> advisory ロック（shared/exclusive、`unlock`）。
> **backup**: メモリ有界ストリーミング、特殊ファイル（FIFO/デバイス/ハードリンク/sparse/xattr/非 UTF-8 名）、
> 除外（`--exclude`/`--exclude-from`/`--exclude-larger-than`/`--exclude-if-present`/`--exclude-caches`/`--one-file-system`）、
...
> **横断**: 全 result/変更系コマンドが `--json`、パスフレーズ供給は `SLUICE_PASSWORD_COMMAND`
> （シークレットマネージャ）＞ `SLUICE_PASSWORD_FILE`（ファイル先頭行）＞ `SLUICE_PASSWORD` ＞
> 対話プロンプト（`copy` 宛先は対応する `SLUICE_DEST_PASSWORD_COMMAND`/`_FILE`/`SLUICE_DEST_PASSWORD`）、
> 安定 exit code（3/10/11/12/13）、
