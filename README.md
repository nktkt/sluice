...
directories, symlinks, FIFOs (named pipes), and hardlinks, with owner (uid/gid),
mode, mtime, and extended attributes preserved — to a local path **or any
S3-compatible object store**. It offers point-in-time snapshots (of one or
...
machine-readable JSON output, and stable exit codes. Backed by 170 tests across
...
- **M1** — local backup + restore (incremental, symlinks, FIFOs, hardlinks, owner/mode/mtime, xattrs, excludes) — ✅
