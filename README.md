...
machine-readable JSON output, and stable exit codes. Backed by 292 tests across
...
The passphrase comes from the stdout of `SLUICE_PASSWORD_COMMAND` (for a secret
manager), else the file named by `SLUICE_PASSWORD_FILE` (its first line, trailing
newline stripped — keeping the secret out of the environment), else the
`SLUICE_PASSWORD` environment variable, else an interactive no-echo prompt. A
repository is a local path or an object-store URL. (`copy`'s destination uses the
matching `SLUICE_DEST_PASSWORD_COMMAND`/`_FILE`/`SLUICE_DEST_PASSWORD`, falling
back to the source passphrase.)
...
# ...or fetch it on demand from a secret manager:
export SLUICE_PASSWORD_COMMAND='pass show backups/sluice'
```

### Create and back up

```sh
sluice init   ./repo                             # refuses if a repository already exists there
...
sluice snapshots ./repo --compact              # one terse line per snapshot (id, date, tags)
...
sluice find      ./repo '**/*.pdf' --tag prod --last 5   # ...only within selected snapshots
...
sluice tag       ./repo --host db1 --add keep    # ...or edit every matching snapshot in bulk
...
sluice key add    ./repo --json           # add/passwd/remove also accept --json (new key id, counts)
```
...
`--path` (the same selectors `snapshots` lists by) and/or `--last N` (the N most
recent of the matched set) — e.g. mirror only your seven most recent
