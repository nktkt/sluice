...
mode, and mtime preserved — to a local path **or any S3-compatible object store**. It offers point-in-time snapshots (of one or
...
machine-readable JSON output, and stable exit codes. Backed by 167 tests across
...
- **M1** — local backup + restore (incremental, symlinks, FIFOs, hardlinks, owner/mode/mtime, excludes) — ✅
